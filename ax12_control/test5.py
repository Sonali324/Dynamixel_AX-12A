...
def get_motor_positions(motor_objects, num_motors):
    """Get current positions of all motors and return as a list"""
    positions = []
    for motor_object in motor_objects:
        positions.append(motor_object.get_goal_position())  # Store goal positions
    
    # Pad positions with default value (e.g., 0) to ensure consistent length
...
def main(motor_objects):
    ...
    while bool_test:
        # Get current goal positions before setting new goal positions
        positions_array.append(get_motor_positions(motor_objects))
        
        speed = int(input("Enter moving speed for all motors: "))
        for motor_object in motor_objects:
            print("\nPosition of dxl ID %d is %d" % (motor_object.id, motor_object.get_goal_position()))
            ...
            #print("Position of dxl ID %d is now: %d" % (motor_object.id, motor_object.get_goal_position()))
